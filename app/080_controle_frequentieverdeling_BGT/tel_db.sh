...
# "*                                                                             *"
# "* Aangeroepen vanuit :      START_SH_tel_db.sh                                *"
# "*                                                                             *"
# "*******************************************************************************"
# "*                                                                             *"
...
#
# in Linux: sh tel_db.sh <DB_SERVER> <DATABASE> <DB_PORT> <DB_USER>
# parameters tussen rechte haken te vervangen door de juiste waarden,
# bijvoorbeeld: tel_db.sh 85.222.225.45 bgt_dev 8080 bgt
...
echo
echo "*******************************************************************************"
echo "* Start tellen aantallen per objectklasse DB ...                              *"
echo "*******************************************************************************"
echo
...
psql -h ${db_server} -d ${database} -p ${db_port} -U ${db_user} -t -c "select 'insert into imgeo_controle.tel_db (select ' || chr(39) || table_name || chr(39) || ' as tabelnaam,' || ' count(*) from '  || 'imgeo.'|| table_name || ');' as smt
from information_schema.tables
where table_schema='imgeo'
and ( table_name like 'bgt_%'
or   table_name like 'imgeo_%')
order by table_name;" 2>&1 | tee ${sqlbestand}
