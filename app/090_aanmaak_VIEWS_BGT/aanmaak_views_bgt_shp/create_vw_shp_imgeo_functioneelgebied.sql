...
\qecho
\qecho '*******************************************************************************'
\qecho '* Aanmaak view imgeo_extractie.vw_shp_imgeo_functioneelgebied ...             *'
\qecho '*******************************************************************************'
\qecho
...
DROP VIEW IF EXISTS imgeo_extractie.vw_shp_imgeo_functioneelgebied;

CREATE OR REPLACE VIEW imgeo_extractie.vw_shp_imgeo_functioneelgebied
AS
(
SELECT identificatie_namespace     as NAMESPACE
     ...
     , case
        ...
        'BGT_FGD_'|| -- prefixed toevoegen aan BESTANDSNAAM
        LOWER(    REPLACE(                   
                REPLACE(
                    REPLACE(
                        REPLACE(
    ...
    when bgt_type ='niet-bgt' and plus_type is null -- cntr op bgt_type ='niet-bgt' en plus_type is leeg
         then 'BGTPLUS_FGD_'||'onbekend' --prefixed en plustype 'onbekend' in BESTANDSNAAM_plus
         else
            case
            when bgt_type ='niet-bgt' -- cntr op bgt_type ='niet-bgt' en plustype is wel gevuld
            then
                 'BGTPLUS_FGD_'|| --prefixed en plustype in BESTANDSNAAM_plus
             LOWER( REPLACE(                   
                    REPLACE(
                        REPLACE(
                            REPLACE(
...
\qecho
\qecho '*******************************************************************************'
\qecho '* Klaar met aanmaak view imgeo_extractie.vw_shp_imgeo_functioneelgebied.      *'
\qecho '*******************************************************************************'
\qecho
