...
\qecho
\qecho '*******************************************************************************'
\qecho '* Aanmaak view imgeo_extractie.vw_imgeo_spoor ...                             *'
\qecho '*******************************************************************************'
\qecho
...
DROP VIEW IF EXISTS imgeo_extractie.vw_imgeo_spoor;

CREATE OR REPLACE VIEW imgeo_extractie.vw_imgeo_spoor
AS
(
SELECT identificatie_namespace     as NAMESPACE
    ...
    when bgt_functie ='niet-bgt' and plus_functie is null -- cntr op bgt_type ='niet-bgt' en plus_type is leeg
         then 'BGTPLUS_SPR_'||'onbekend' --prefixed en plustype 'onbekend' in BESTANDSNAAM_plus
         else
            case
            when bgt_functie ='niet-bgt' -- cntr op bgt_type ='niet-bgt' en plustype is wel gevuld
            then
                 'BGTPLUS_SPR_'|| --prefixed en plustype in BESTANDSNAAM_plus
        LOWER(        REPLACE(                   
                    REPLACE(
                        Replace(
                            REPLACE(
...
\qecho
\qecho '*******************************************************************************'
\qecho '* Klaar met aanmaak view imgeo_extractie.vw_imgeo_spoor.                      *'
\qecho '*******************************************************************************'
\qecho
